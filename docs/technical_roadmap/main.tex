...
\begin{enumerate}
  ...
  \item 人机共享空间分为 Phase 4a 进入检测停止、Phase 4b 受约束动态绕行；优先验证停止，再增加在线重规划，见第~\ref{ch:dynamic-perception}~章。
...
\input{shared_workspace_perception}
...
以下 P0--P7 为技术工作包；项目 Stage/Phase 4 为端到端受监督演示，其 4a（进入即停）和 4b（动态绕行）见第~\ref{ch:dynamic-perception}~章。感知及停止链路在固定平台阶段提前实施。
...
P4 & 感知基线 & RGB/RGB-D 对照、标定、工位相对路点、有效性门禁 & 留出姿态误差满足容差；E2 对照及异常观测拒绝 \\

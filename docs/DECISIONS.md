...
- Verification: On 2026-08-29, image `sha256:651e139692da8839bd47a73d8410e5fcc0eb6a9524c64003df8479a2f400738b` built once with the pinned vendor commit. All eight baseline packages compiled; the six-axis `uf850` Xacro passed `check_urdf`; and a 35-second no-hardware smoke run reached MoveIt's ready state and activated `joint_state_broadcaster` and `uf850_traj_controller`. Resolved key packages were `gz_ros2_control` 1.2.19, MoveIt 2.12.4, `ros_gz` 1.0.22 and `ros2_controllers` 4.40.1.
- Boundary: The official launch always starts graphical clients. RViz exited under the container's offscreen smoke setup while Gazebo server, MoveIt and both controllers started, so a clean display-independent ATOM launch and repeatability runs remain pending. This decision does not validate the delivered controller firmware, network configuration, serial-number calibration, payload/TCP, custom gripper mount or any hardware motion.
